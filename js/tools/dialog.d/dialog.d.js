...
(function(global) {
    ...
    D.dialog = {
        tpl: {
            ...
            modal:'<div class="d-modal-cell"><span class="d-modal-close">X</span><div class="d-modal-header"><&=header&></div><div class="d-modal-content"><&=content&></div><div class="d-modal-footer"><&=footer&></div></div>'
        },
        config: {
            alert: {
                title: '',
    ...
    D.modal=function(param,cb){
        var utils = this.utils;
        var dialog = D.dialog;
        var modalTpl = dialog.tpl.modal;
        var opt = utils.extend(dialog.config.modal,param);
        var modalEle = document.querySelector('#'+opt.modalId);
        dialog.createMask();
        if(!modalEle){
            modalEle=dialog.createBox();
            modalEle.id = opt.modalId;
            modalEle.innerHTML = utils.render(modalTpl, opt);
            document.body.appendChild(modalEle);
            utils.setEvent(modalEle.querySelectorAll('.d-modal-close')[0], 'click', function(e) {
                e.stopPropagation();
                dialog.removeMask();
                modalEle.className = 'd-dialog-box d-hide ' + opt.animateHide;
            });
            console.log(cb);
            if (cb) {cb();}
        }else{
            modalEle.className = 'd-dialog-box d-show ' + opt.animateShow;
        }
    };
    D.utils = { //工具类
        version: '1.0.0',
